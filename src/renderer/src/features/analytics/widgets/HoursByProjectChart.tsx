...
export default function HoursByProjectChart({ sessionData }: WidgetProps): React.JSX.Element {
  const { textColor, palette } = useChartColors()

  const data = useMemo(() => {
    const map = new Map<string, number>()
    for (const s of sessionData) {
      map.set(s.projectName, (map.get(s.projectName) ?? 0) + s.durationMinutes / 60)
    }
    return [...map.entries()]
