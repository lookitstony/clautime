...
export default function BillableEarningsChart({ summaryData }: WidgetProps): React.JSX.Element {
  ...
  if (data.length === 0) return <div className="flex h-full items-center justify-center text-sm text-[var(--text-muted)]">No billable data</div>
      ...
      <BarChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
        ...
        <Tooltip
          ...
          content={({ active, payload }) => {
            ...
            const { hours, rate, cost } = payload[0].payload as { hours: number; rate: number; cost: number }
            return (
