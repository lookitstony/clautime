...
const CATEGORIES = [
  ...
  { id: 'widgets', label: 'Widgets & Alerts', icon: Bell },
...
export function SettingsPage(): React.JSX.Element {
  ...
  const removeUnconfiguredFromCache = useCallback(
    (email: string) => {
      queryClient.setQueryData<UnconfiguredAuthor[]>(['git', 'unconfiguredEmails'], (old) =>
        Array.isArray(old) ? old.filter((a) => a.email.toLowerCase() !== email.toLowerCase()) : old
      )
    },
    [queryClient]
  )
  ...
  return (
    <div className="mx-auto flex max-w-4xl gap-8 p-6">
      {/* Category rail */}
      <nav className="sticky top-6 h-max w-44 shrink-0">
        <h1 className="mb-4 px-2.5 text-[18px] font-bold text-[var(--text-primary)]">Settings</h1>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    active
                      ...
                      : 'text-[var(--text-secondary)] hover:bg-[var(--surface-border)]/40 hover:text-[var(--text-primary)]'
                  )}
                >
      ...
      <div className="min-w-0 flex-1 space-y-6">
              ...
              {showAiInstructions && (
                <>
                  <p className="mt-2 mb-2 text-[11px] text-[var(--text-muted)]">
                    Customize the prompt used when generating AI work summaries. Commit data and
                    section formatting are added automatically — these instructions control tone and
                    style.
                  </p>
                  <textarea
                    value={aiInstructions}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  ...
                  {showAiBriefInstructions &&
                    aiBriefInstructions !== DEFAULT_AI_BRIEF_INSTRUCTIONS && (
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => {
        ...
        {activeCategory === 'invoicing' && (
          <section>
            ...
            <SectionCard>
              <p className="mb-3 text-[12px] text-[var(--text-muted)]">
                Add Stripe secret keys to enable invoicing directly from ClauTime. Toggle between
            ...
            <SectionCard>
              <div>
                ...
                <p className="mb-2 text-[11px] text-[var(--text-muted)]">
                  Used for &ldquo;This Week&rdquo; and &ldquo;Last Week&rdquo; filters across
                  sessions, reports, and analytics.
                </p>
                <div className="flex gap-1">
                  {(
                    [
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        saveSetting.mutate({ key: 'week_start_day', value: opt.value })
                      }
                      className={cn(
            ...
            <SectionCard>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-[var(--text-muted)]">
                    Show each project&apos;s Stage Name instead of its real name across Sessions,
                    Live, and Analytics — for streaming or demos. Set stage names per project in
                    Clients. Also toggleable from the title bar (mask icon).
                  </p>
                </div>
                <Switch
            ...
            <SectionCard>
              <div>
                ...
                <p className="mb-2 text-[11px] text-[var(--text-muted)]">
                  ...
                  under, separated by commas — including GitHub <code>noreply</code> addresses.
                  Commits authored under an email not listed here are silently skipped.
                </p>
                <div className="flex items-center gap-2">
                  <input
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (detectedGitIdentity?.email)
                        addUnconfiguredEmail(detectedGitIdentity.email)
                    }}
              ...
              {unconfiguredEmails.length > 0 && (
                <div className="mt-4 space-y-2 rounded-md border border-amber-500/40 bg-amber-500/10 p-3">
                  ...
                  <p className="text-[11px] text-[var(--text-muted)]">
                    These emails appear in your repos&apos; recent history but aren&apos;t
                    configured, so their commits are being skipped. Add the ones that are you.
                  </p>
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <div className="truncate text-[11px] text-[var(--text-muted)]">
                            {presentationMode ? '••••••' : author.name || 'unknown'} ·{' '}
                            {author.count} commit
                            {author.count === 1 ? '' : 's'}
                          </div>
                        </div>
            ...
            <SectionCard>
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-[11px] text-[var(--text-muted)]">
                    Hide widgets when idle timeout is reached, show them again when activity
                    resumes.
                  </p>
                </div>
                <Switch
        ...
        {activeCategory === 'privacy' && (
          <section>
            ...
            <SectionCard>
              <p className="mb-3 text-[12px] text-[var(--text-muted)]">
                Scan JSONL conversation files for accidentally exposed secrets like API keys,
              ...
              {scanSummary && scanSummary.total > 0 && (
                <div className="mb-3 rounded border border-[var(--surface-border)] bg-[var(--background-primary)] px-3 py-2 text-[12px] text-[var(--text-secondary)]">
                  ...
                  {' | '}
                  <span>
                    {scanSummary.found} active, {scanSummary.redacted} redacted,{' '}
                    {scanSummary.ignored} ignored
                  </span>
                </div>
              )}
