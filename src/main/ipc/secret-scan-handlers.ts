...
export function registerSecretScanHandlers(): void {
  ipcMain.handle(
    'secretScan:run',
    async (): Promise<IpcResult<SecretScanResult>> => {
      try {
  ...
  ipcMain.handle(
    'secretScan:getSummary',
    async (): Promise<IpcResult<SecretScanSummary>> => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, pattern: unknown): Promise<IpcResult<{ success: boolean; warnings: string[] }>> => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, source: unknown, flags: unknown, testString: unknown): Promise<IpcResult<PatternTestResult>> => {
      try {
